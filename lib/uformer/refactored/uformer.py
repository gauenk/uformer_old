...
class Uformer(nn.Module):
    def __init__(self, img_size=128, in_chans=3,
                 ...
                 dowsample=Downsample, upsample=Upsample, refactored=True, **kwargs):
        super().__init__()
        ...
        self.encoderlayer_0 = BasicUformerLayer(dim=embed_dim,
                            ...
                            use_checkpoint=use_checkpoint,
                                                token_projection=token_projection,token_mlp=token_mlp,se_layer=se_layer,refactored=refactored)
    ...
    def forward(self, x, mask=None, coords=None):
        ...
        y = self.pos_drop(y)
        #Encoder
        conv0 = self.encoderlayer_0(y,mask=mask,coords=coords)
        pool0 = self.dowsample_0(conv0)
        conv1 = self.encoderlayer_1(pool0,mask=mask,coords=coords)
        pool1 = self.dowsample_1(conv1)
        conv2 = self.encoderlayer_2(pool1,mask=mask,coords=coords)
        pool2 = self.dowsample_2(conv2)
        conv3 = self.encoderlayer_3(pool2,mask=mask,coords=coords)
        pool3 = self.dowsample_3(conv3)

        # Bottleneck
        conv4 = self.conv(pool3, mask=mask,coords=coords)

        #Decoder
        up0 = self.upsample_0(conv4)
        deconv0 = torch.cat([up0,conv3],-1)
        deconv0 = self.decoderlayer_0(deconv0,mask=mask,coords=coords)

        up1 = self.upsample_1(deconv0)
        deconv1 = torch.cat([up1,conv2],-1)
        deconv1 = self.decoderlayer_1(deconv1,mask=mask,coords=coords)

        up2 = self.upsample_2(deconv1)
        deconv2 = torch.cat([up2,conv1],-1)
        deconv2 = self.decoderlayer_2(deconv2,mask=mask,coords=coords)

        up3 = self.upsample_3(deconv2)
        deconv3 = torch.cat([up3,conv0],-1)
        deconv3 = self.decoderlayer_3(deconv3,mask=mask,coords=coords)
