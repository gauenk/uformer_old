...
# -- network --
import uformer
from uformer.utils.misc import optional,rslice_pair,expand2square
...
def run_exp(cfg):
    ...
    if cfg.model_type == "original":
        model = uformer.original.load_model(noise_version="blur",
                                            modulator=True).to(cfg.device)
        ...
        fwd_mode = "dnls_k"
        model = uformer.augmented.load_model(cfg.sigma,fwd_mode=fwd_mode,
                                             stride=cfg.stride,noise_version="blur")
    else:
    ...
    indices = [i for i,g in enumerate(groups) if cfg.vid_name in g.split("_")[0]]
    ...
    for index in indices:
        ...
        with th.no_grad():
            ...
            for t in range(noisy.shape[0]):
                vshape = noisy[[t]].shape
                noisy_t,mask_t = expand2square(noisy[[t]],factor=128)
                deno_t = model(noisy_t/imax)
                deno_t = th.masked_select(deno_t,mask_t).reshape(vshape)
                deno.append(deno_t)
            deno = th.cat(deno)
...
def main():
    ...
    dnames = ["gopro"]
    dset = ["te"]
    vid_names = ["%02d" % x for x in np.arange(0,11)]
    ...
    for model_type,mdf in records.groupby("model_type"):
        for stride,sdf in mdf.groupby("stride"):
            ...
            psnrs_m = np.concatenate(psnrs_m)
            ssims_m = np.concatenate(ssims_m)
            print(model_type,stride,np.mean(psnrs_m),np.mean(ssims_m))
